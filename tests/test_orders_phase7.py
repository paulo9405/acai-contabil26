...
def _order(db, *, order_date=None, payment_method=Order.PaymentMethod.PIX, total='18.00',
...
def _item(order, *, product_name='Produto A', size_name='300 ml', quantity=1,
          line_total='18.00', variant=None):
...
class TestGetSalesByPaymentMethod:
    ...
    def test_excludes_cancelled(self, db):
        _order(db, payment_method=Order.PaymentMethod.CARD, total='50.00', status=Order.Status.CANCELLED)
...
class TestGetLitersSold:
    def test_calculates_liters_via_variant_size(self, db):
        ...
        o = _order(db)
        OrderItemFactory(order=o, variant=variant, quantity=2,
                         size_name='500 ml', product_name='Açaí',
                         unit_price=Decimal('24.00'), addons_total=Decimal('0'),
    ...
    def test_excludes_cancelled_orders(self, db):
        ...
        o = _order(db, status=Order.Status.CANCELLED)
        OrderItemFactory(order=o, variant=variant, quantity=3,
                         size_name='1 litro', product_name='Açaí',
                         unit_price=Decimal('38.00'), addons_total=Decimal('0'),
    ...
    def test_items_without_variant_excluded(self, db):
        o = _order(db)
        OrderItemFactory(order=o, variant=None, quantity=5,
                         size_name='1 litro', product_name='Açaí',
                         unit_price=Decimal('38.00'), addons_total=Decimal('0'),
...
class TestGetTopAddons:
    def test_ranks_by_count(self, db):
        o = _order(db)
        item = _item(o)
        OrderItemAddonFactory(order_item=item, name='Granola', is_included=True, line_total=Decimal('0'))
        OrderItemAddonFactory(order_item=item, name='Granola', is_included=False, line_total=Decimal('3.00'))
        OrderItemAddonFactory(order_item=item, name='Nutella', is_included=False, line_total=Decimal('7.50'))
...
class TestGetPeakHours:
    def test_groups_by_hour(self, db):
        user = UserFactory()
        OrderFactory(created_by=user, order_date=TODAY,
                     order_time=time(14, 0), total=Decimal('18.00'))
        OrderFactory(created_by=user, order_date=TODAY,
                     order_time=time(14, 30), total=Decimal('18.00'))
        OrderFactory(created_by=user, order_date=TODAY,
                     order_time=time(16, 0), total=Decimal('18.00'))
    ...
    def test_ordered_by_hour(self, db):
        user = UserFactory()
        OrderFactory(created_by=user, order_date=TODAY,
                     order_time=time(18, 0), total=Decimal('18.00'))
        OrderFactory(created_by=user, order_date=TODAY,
                     order_time=time(11, 0), total=Decimal('18.00'))
...
class TestGetDivergences:
    def test_returns_orders_with_divergence(self, db):
        user = UserFactory()
        o = OrderFactory(created_by=user, order_date=TODAY, total=Decimal('18.00'),
    ...
    def test_excludes_matching_total(self, db):
        user = UserFactory()
        OrderFactory(created_by=user, order_date=TODAY, total=Decimal('18.00'),
