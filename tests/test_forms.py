...
from tests.conftest import ExpenseCategoryFactory
...
class TestExpenseForm:
    ...
    def test_form_invalid_future_date(self, expense_category):
        ...
        form = ExpenseForm(
            data={"date": future_date, "category": expense_category.id, "amount": "50.00"}
        )
    ...
    def test_form_invalid_zero_amount(self, expense_category):
        ...
        form = ExpenseForm(
            data={"date": timezone.now().date(), "category": expense_category.id, "amount": "0.00"}
        )
        assert not form.is_valid()
        assert "amount" in form.errors
...
class TestExpenseFilterForm:
    ...
    def test_form_valid_date_range(self):
        ...
        form = ExpenseFilterForm(data={"date_from": today - timedelta(days=7), "date_to": today})
        assert form.is_valid()

    def test_form_invalid_date_range(self):
        """Testa que data_from > date_to é inválido."""
        today = timezone.now().date()
        form = ExpenseFilterForm(data={"date_from": today, "date_to": today - timedelta(days=7)})
        assert not form.is_valid()
        assert form.non_field_errors()

    def test_form_with_category(self, expense_category):
        """Testa filtro por categoria."""
        form = ExpenseFilterForm(data={"category": expense_category.id})
        assert form.is_valid()
...
class TestReportFilterForm:
    ...
    def test_form_valid_custom_period(self):
        ...
        form = ReportFilterForm(
            data={"period": "custom", "start_date": today - timedelta(days=7), "end_date": today}
        )
        assert form.is_valid()

    def test_form_invalid_custom_without_start_date(self):
        """Testa custom sem data inicial."""
        form = ReportFilterForm(data={"period": "custom", "end_date": timezone.now().date()})
        assert not form.is_valid()
        assert "start_date" in form.errors

    def test_form_invalid_custom_without_end_date(self):
        """Testa custom sem data final."""
        form = ReportFilterForm(data={"period": "custom", "start_date": timezone.now().date()})
        assert not form.is_valid()
    ...
    def test_form_invalid_custom_date_range(self):
        ...
        today = timezone.now().date()
        form = ReportFilterForm(
            data={"period": "custom", "start_date": today, "end_date": today - timedelta(days=7)}
        )
        assert not form.is_valid()
