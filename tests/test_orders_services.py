...
from orders.services import create_order, cancel_order, update_order
from orders.selectors import get_orders_by_date, get_order_detail
from tests.conftest import (
    UserFactory, ProductCategoryFactory, SizeFactory,
    ProductFactory, ProductVariantFactory, AddonFactory,
)
...
class TestOrderModel:
    ...
    def test_has_total_divergence_equal(self, user):
        from tests.conftest import OrderFactory
        order = OrderFactory(created_by=user, total=Decimal('30.00'), informed_total=Decimal('30.00'))
        assert order.has_total_divergence is False

    def test_has_total_divergence_different(self, user):
        from tests.conftest import OrderFactory
        order = OrderFactory(created_by=user, total=Decimal('30.00'), informed_total=Decimal('28.00'))
...
class TestCreateOrder:
    ...
    def test_dois_adicionais_dentro_limite(self, user, variant_build_300, addon_free1, addon_free2):
        ...
        order = create_order(
            items=[{'variant': variant_build_300, 'quantity': 1, 'addons': [addon_free1, addon_free2]}],
            **_base_order_kwargs(user),
        )
        item = order.items.first()
    ...
    def test_excedente_vira_pago_automaticamente(self, user, variant_build_300, addon_free1, addon_free2, addon_free3=None):
        ...
        order = create_order(
            items=[{'variant': variant_build_300, 'quantity': 1, 'addons': [addon_free1, addon_free2, addon_free3]}],
            **_base_order_kwargs(user),
        )
        item = order.items.first()
        assert item.addons.filter(is_included=True).count() == 2
...
class TestUpdateOrder:
    ...
    def test_atualiza_payment_method(self, user, variant_standard):
        order = self._create_simple_order(user, variant_standard)
        updated = update_order(order=order, updated_by=user, payment_method=Order.PaymentMethod.CASH)
...
class TestSelectors:
    ...
    def test_get_orders_by_date(self, user, variant_standard):
        d = date(2026, 7, 15)
        o1 = create_order(
            comanda_number='1', order_date=d, order_time=time(14, 0),
            payment_method=Order.PaymentMethod.PIX, created_by=user,
            items=[{'variant': variant_standard, 'quantity': 1, 'addons': []}],
        )
        o2 = create_order(
            comanda_number='2', order_date=d, order_time=time(16, 0),
            payment_method=Order.PaymentMethod.CASH, created_by=user,
            items=[{'variant': variant_standard, 'quantity': 1, 'addons': []}],
        )
        # Pedido de outro dia — não deve aparecer
        create_order(
            comanda_number='3', order_date=date(2026, 7, 14), order_time=time(10, 0),
            payment_method=Order.PaymentMethod.PIX, created_by=user,
            items=[{'variant': variant_standard, 'quantity': 1, 'addons': []}],
        )
